...
const migrateToGridFS = async () => {
    try {
        ...
        if (!mongoURI || mongoURI.includes("localhost")) {
             console.log("CRITICAL ERROR: MONGO_URI is set to localhost. Please add your Atlas URI to .env first!");
             console.log("Current URI:", mongoURI);
             process.exit(1);
        }
        ...
        console.log("Cleaning existing food items...");
        await foodModel.deleteMany({});
        
        const assetsPath = path.join(__dirname, "..", "frontend", "src", "assets");

        for (const item of foodData) {
            const imagePath = path.join(assetsPath, item.image);
            
            if (fs.existsSync(imagePath)) {
                console.log(`Uploading ${item.image} for ${item.name}...`);
                
                const gridfsFilename = `${Date.now()}_${item.image}`;
                ...
                console.warn(`SKIPPING: Image ${item.image} not found in assets folder.`);
            }
        }

        console.log("\nALL DONE! Your live website is now fully populated with images.");
        process.exit();
    } catch (error) {
